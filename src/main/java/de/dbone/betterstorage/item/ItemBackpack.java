...
import de.dbone.betterstorage.client.model.ModelBackpack;
...
import de.dbone.betterstorage.container.SlotArmorBackpack;
...
import de.dbone.betterstorage.inventory.InventoryStacks;
...
import de.dbone.betterstorage.utils.EntityUtils;
import de.dbone.betterstorage.utils.GuiHandler;
import de.dbone.betterstorage.utils.LanguageUtils;
...
public class ItemBackpack extends ItemArmorBetterStorage implements ISpecialArmor, IDyeableItem {
	...
	public static final ArmorMaterial material = EnumHelper.addArmorMaterial("backpack", "error?", 14, new int[]{ 0, 2, 0, 0 }, 15);
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ) {
		EnumFacing orientation = DirectionUtils.getOrientation(playerIn).getOpposite();
		return placeBackpack(playerIn, playerIn, stack, pos.getX(), pos.getY(), pos.getZ(), 1/*side*/, orientation, false, false);
	}
	...
	public static boolean onPlaceBackpack(EntityPlayer player, int x, int y, int z, EnumFacing side) {
	...
	public static boolean placeBackpack(EntityLivingBase carrier, EntityPlayer player, ItemStack backpack,
	                                    int x, int y, int z, int side, EnumFacing orientation, boolean despawn, boolean deathDrop) {
		...
		if (!world.getBlockState(new BlockPos(x, y, z)).getBlock().isReplaceable(world, new BlockPos(x, y, z))) {
		...
		if ((deathDrop ? blockBelow.isReplaceable(world, new BlockPos(x, y - 1, z))
		               : !world.isSideSolid(new BlockPos(x, y - 1, z), EnumFacing.UP))) return false;
		
		// Return false if there's an entity blocking the placement.
		if (!world.canBlockBePlaced(blockBackpack, new BlockPos(x, y, z), deathDrop, orientation, carrier, backpack)) return false;
		
		// Return false if the player can't edit the block.
		if ((player != null) && (!world.canMineBlockBody(player, new BlockPos(x, y, z)) ||
		                         !player.canPlayerEdit(new BlockPos(x, y, z), orientation, backpack))) return false;
		...
		if (world.getBlockState(new BlockPos(x, y, z)).getBlock() != blockBackpack)
			return false;
		
		blockBackpack.onBlockPlacedBy(world, new BlockPos(x, y, z), world.getBlockState(new BlockPos(x, y, z)), carrier, backpack);
		//blockBackpack.onPostBlockPlaced(world, new BlockPos(x, y, z), orientation.ordinal());
		
		TileEntityBackpack te = WorldUtils.get(world, new BlockPos(x, y, z), TileEntityBackpack.class);
		te.stack = backpack.copy();
		if (ItemBackpack.getBackpack(carrier) == backpack)
			te.unequip(carrier, despawn);
		
		String sound = blockBackpack.stepSound.soundName;
