...
import de.dbone.betterstorage.api.BetterStorageBackpack;
...
import de.dbone.betterstorage.utils.ReflectionUtils;
...
public class BackpackHandler {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (event.action == Action.RIGHT_CLICK_BLOCK)
			if (ItemBackpack.onPlaceBackpack(event.entityPlayer, event.pos.getX(), event.pos.getY(), event.pos.getZ(), event.face)) {
	...
	public void onSpecialSpawn(SpecialSpawn event) {
		...
		if (entity.getClass().equals(EntityEnderman.class)) {
			if ((BetterStorageTiles.enderBackpack != null) &&
			    ...
			    (world.getBiomeGenForCoords(new BlockPos(entity.posX, entity.posY, entity.posZ)) != BiomeGenBase.sky)) {
		...
		if (backpack == null) {
			...
			if (backpackData.spawnsWithBackpack) {
				...
				if (contents != null) {
					...
					WeightedRandomChestContent.generateChestContents(RandomUtils.random, Arrays.asList(randomBackpackItems), inventory, 20);
	...
	public void onLivingDeath(LivingDeathEvent event) {
		...
		boolean keepInventory = entity.worldObj.getGameRules().getBoolean("keepInventory");
			...
			if (BetterStorage.globalConfig.getBoolean(GlobalConfig.dropBackpackOnDeath)) {
				...
				while (!coords.isEmpty()) {
					...
					while (iter.hasNext()) {
						...
						boolean replacable = entity.worldObj.getBlockState(new BlockPos(coord.x, coord.y, coord.z)).getBlock()
								.isReplaceable(entity.worldObj, new BlockPos(coord.x, coord.y, coord.z));
