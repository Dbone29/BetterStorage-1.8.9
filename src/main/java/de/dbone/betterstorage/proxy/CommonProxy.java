...
import de.dbone.betterstorage.item.IDyeableItem;
import de.dbone.betterstorage.item.ItemBackpack;
import de.dbone.betterstorage.item.ItemBucketSlime;
import de.dbone.betterstorage.item.cardboard.ICardboardItem;
import de.dbone.betterstorage.item.cardboard.ItemCardboardSheet;
import de.dbone.betterstorage.misc.ChristmasEventHandler;
import de.dbone.betterstorage.misc.EquipmentSlot;
import de.dbone.betterstorage.misc.handlers.BackpackHandler;
...
public class CommonProxy {
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (leftClick || rightClick) {
			...
			IHasAttachments hasAttachments = WorldUtils.get(world, pos, IHasAttachments.class);
		...
		if (rightClick && (block == Blocks.cauldron)) {
			...
			if (metadata > 0) {
				...
				if ((dyeable != null) && (dyeable.canDye(holding))) {
					...
					//world.setBlockMetadataWithNotify(pos, metadata - 1, 2);
		...
		if (!world.isRemote && BetterStorageTiles.lockableDoor != null && rightClick && block == Blocks.iron_door) {
			...
			if(target != null && getIronDoorHightlightBox(player, world, pos.getX(), pos.getY(), pos.getZ(), target.hitVec, block) != null) {
				...
				if(meta >= 8) {
					isMirrored = meta == 9;
					//y -= 1;
					meta = world.getBlockState(pos).getBlock().getMetaFromState(world.getBlockState(pos));
				}
				else isMirrored = world.getBlockState(pos).getBlock().getMetaFromState(world.getBlockState(pos)) == 9;//world.getBlockMetadata(x, y + 1, z)
				
				int rotation = meta & 3;
				EnumFacing orientation = rotation == 0 ? EnumFacing.WEST : rotation == 1 ? EnumFacing.NORTH : rotation == 2 ? EnumFacing.EAST : EnumFacing.SOUTH;
				orientation = isMirrored ? (
						orientation == EnumFacing.WEST ? EnumFacing.SOUTH : 
						orientation == EnumFacing.NORTH ? EnumFacing.WEST : 
						orientation == EnumFacing.EAST ? EnumFacing.NORTH : 
							EnumFacing.EAST) : orientation;
						
				world.setBlockState(pos, BetterStorageTiles.lockableDoor.getDefaultState(), 0);
				world.setBlockState(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ()), BetterStorageTiles.lockableDoor.getDefaultState(), 8);
						
				world.setBlockState(pos, BetterStorageTiles.lockableDoor.getDefaultState());
				BlockPos test = new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ());//world.setBlock(x, y, z, BetterStorageTiles.lockableDoor, 0, SetBlockFlag.SEND_TO_CLIENT);
				world.setBlockState(test, BetterStorageTiles.lockableDoor.getDefaultState());//world.setBlock(x, y + 1, z, BetterStorageTiles.lockableDoor, 8, SetBlockFlag.SEND_TO_CLIENT);
	...
	protected AxisAlignedBB getIronDoorHightlightBox(EntityPlayer player, World world, int x, int y, int z, Vec3 hitVec, Block block) {
		...
		if(meta >= 8) {
			isMirrored = meta == 9;
			y -= 1;
			meta = world.getBlockState(new BlockPos(x, y, z)).getBlock().getMetaFromState(world.getBlockState(new BlockPos(x, y, z)));
		}
		else isMirrored = world.getBlockState(new BlockPos(x, y + 1, z)).getBlock().getMetaFromState(world.getBlockState(new BlockPos(x, y + 1, z))) == 9;
