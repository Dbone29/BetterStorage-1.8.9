...
public abstract class TileEntityLockable extends TileEntityConnectable implements ILockable, IHasAttachments {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!worldObj.isRemote && canHaveLock() &&!canPlayerUseContainer(playerIn))
			((ILock)getLock().getItem()).applyEffects(getLock(), this, playerIn, EnumLockInteraction.OPEN);
	...
	@Override
	public void useUnlocked(EntityPlayer playerIn, World worldIn, BlockPos pos) {
	...
	public void setPowered(boolean powered) {
		...
		if (powered) worldObj.scheduleBlockUpdate(pos, block, 0, 10);
		
		// Notify nearby blocks
		//TODO: Testing
		worldObj.notifyNeighborsOfStateChange(pos, block);
		/*worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY(), pos.getZ(), block);
		worldObj.notifyBlocksOfNeighborChange(pos.getX() - 1, pos.getY(), pos.getZ(), block);
		worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY() + 1, pos.getZ(), block);
		worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY() - 1, pos.getZ(), block);
		worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY(), pos.getZ() + 1, block);
		worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY(), pos.getZ() - 1, block);
		
		// Notify nearby blocks of adjacent chest
		if (isConnected() && (getConnected() == EnumFacing.EAST)) {
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 2, pos.getY(), pos.getZ(), block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY() + 1, pos.getZ(), block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY() - 1, pos.getZ(), block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY(), pos.getZ() + 1, block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY(), pos.getZ() - 1, block);
		}
		if (isConnected() && (getConnected() == EnumFacing.SOUTH)) {
			worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY(), pos.getZ() + 2, block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() + 1, pos.getY(), pos.getZ() + 1, block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX() - 1, pos.getY(), pos.getZ() + 1, block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY() + 1, pos.getZ() + 1, block);
			worldObj.notifyBlocksOfNeighborChange(pos.getX(), pos.getY() - 1, pos.getZ() + 1, block);
		}*/
